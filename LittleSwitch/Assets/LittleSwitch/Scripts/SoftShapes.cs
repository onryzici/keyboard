...
public static class SoftShapes {
 ...
 // Repeated switches and sockets share geometry; editor mesh authoring still gets an independent mesh.
 static readonly Dictionary<(Vector3 size,float radius),Mesh> runtimeMeshes=new Dictionary<(Vector3,float),Mesh>();
 public static Color C(string hex){ColorUtility.TryParseHtmlString("#"+hex,out var c);return c;}
 public static Material Mat(string hex){if(mats.TryGetValue(hex,out var m)&&m)return m;m=new Material(Shader.Find("HDRP/Lit")){name="Mat_"+hex,color=C(hex),enableInstancing=true};m.SetFloat("_Smoothness",.18f);UnityEngine.Rendering.HighDefinition.HDMaterial.ValidateMaterial(m);mats[hex]=m;return m;}
 public static Mesh Rounded(Vector3 size,float r){
  r=Mathf.Min(r,Mathf.Min(size.x,Mathf.Min(size.y,size.z))*.49f);
  var key=(size,r);if(Application.isPlaying&&runtimeMeshes.TryGetValue(key,out var cached)&&cached)return cached;
  ...
  }var mesh=new Mesh{name="SoftBox"};mesh.SetVertices(v);mesh.SetNormals(n);mesh.SetUVs(0,uv);mesh.SetTriangles(tris,0);mesh.RecalculateBounds();if(Application.isPlaying)runtimeMeshes[key]=mesh;return mesh;
 }
