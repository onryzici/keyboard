...
public static class FocusWorkshopReview
{
    ...
    public static void Open() => EditorSceneManager.OpenScene(WorkshopFocusPass.ScenePath);
    ...
    static System.Collections.IEnumerator AssemblyCheck()
    {
        ...
        target=new RenderTexture(1600,900,24,RenderTextureFormat.ARGB32,RenderTextureReadWrite.sRGB);camera.targetTexture=target;
        yield return new WaitForSeconds(2);Save("focus-assembly");
        File.WriteAllText("Logs/focus-terminal.txt","PASS: separate computer order mode returns to walking; main bench assembly camera rendered.");
    ...
    static void Place()
    {
        ...
        Vector3[] aims={new(-.6f,1.45f,2.8f),new(-.9f,1.45f,3.8f),new(4.9f,1.35f,1.2f),new(4.95f,1.2f,-2.5f),new(-.1f,1.1f,-3.5f),new(-5.25f,2.0f,1.7f)};
        walker.transform.position=WorkshopFocusPass.Map(positions[index]);cc.enabled=true;
        camera.transform.rotation=Quaternion.LookRotation(WorkshopFocusPass.Map(aims[index])-camera.transform.position);
    }
    static void Update()
    {
        ...
        try
        {
            if(walking)
            {
                ...
                Vector3 direction=WorkshopFocusPass.Map(Route[index])-current;direction.y=0;
                ...
                camera.transform.rotation=Quaternion.LookRotation(WorkshopFocusPass.Map(Look[index])-camera.transform.position);
            }
            if(++frames<75)return;
            Save((walking?"focus-route-":"focus-view-")+index);
            index++;frames=0;segmentStarted=EditorApplication.timeSinceStartup;
            if(index<(walking?Route.Length:6)){if(!walking)Place();return;}
            Validate();
            File.WriteAllText("Logs/"+(walking?"focus-route-complete":"focus-capture-complete")+".txt",$"{DateTime.UtcNow:O}\nDistance={distance/4.3f:F2}m\nWaypoints={index}\n");
            Cleanup();EditorApplication.isPlaying=false;
        }
        catch(Exception e){File.WriteAllText("Logs/focus-review-error.txt",e.ToString());Debug.LogException(e);Cleanup();EditorApplication.isPlaying=false;}
    }
    ...
    public static void BuildWindows()
    {
        ...
        var report=BuildPipeline.BuildPlayer(new BuildPlayerOptions{scenes=new[]{WorkshopFocusPass.ScenePath},locationPathName="../Builds/WalkableWorkshop/Little Switch Workshop.exe",target=BuildTarget.StandaloneWindows64,options=BuildOptions.None});
        File.WriteAllText("Logs/focus-build.txt",report.summary.result+"\nErrors="+report.summary.totalErrors);
