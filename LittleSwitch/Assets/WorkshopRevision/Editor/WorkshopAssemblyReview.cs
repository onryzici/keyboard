...
public static class WorkshopAssemblyReview {
 static InputSettings originalSettings;static Mouse mouse;static WorkshopVisit visit;static RenderTexture target;
 ...
 static void Exit(PlayModeStateChange state){if(state!=PlayModeStateChange.EnteredEditMode)return;ShopSave.EditorSavePathOverride=null;if(originalSettings)InputSystem.settings=originalSettings;if(mouse!=null)InputSystem.RemoveDevice(mouse);EditorApplication.playModeStateChanged-=Exit;}
 ...
 static IEnumerator Test(){
  ...
  for(int stage=0;stage<2;stage++){
   ...
   for(int i=0;i<61;i++){
    ...
    MouseAt(shop.board.SlotPosition(i),false);yield return new WaitForSeconds(.6f);
  ...
  shop.viewCamera.targetTexture=null;target.Release();Object.Destroy(target);InputSystem.settings=originalSettings;EditorApplication.isPlaying=false;
 }
