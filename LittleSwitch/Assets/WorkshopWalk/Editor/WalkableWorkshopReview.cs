...
public static class WalkableWorkshopReview
{
    ...
    public static void Open() => EditorSceneManager.OpenScene(WalkableWorkshopBuilder.ScenePath);
    ...
    static void Place()
    {
        ...
        walker.transform.position=positions[index];cc.enabled=true;
        camera.transform.rotation=Quaternion.LookRotation(aims[index]-camera.transform.position);
    }
    static void Update()
    {
        ...
        try
        {
            if(walking)
            {
                ...
                Vector3 direction=Route[index]-current;direction.y=0;
                if(direction.magnitude>.16f && frames==0)
                {
                    ...
                    walker.enabled=true;Cursor.lockState=CursorLockMode.Locked;Cursor.visible=false;walker.transform.rotation=Quaternion.LookRotation(direction);
                    walker.reviewMovement=Vector2.up;return;
                }
                walker.reviewMovement=Vector2.zero;walker.enabled=false;
                camera.transform.rotation=Quaternion.LookRotation(Look[index]-camera.transform.position);
            }
            if(++frames<75)return;
            Save((walking?"walk-route-":"walk-view-")+index);
            index++;frames=0;segmentStarted=EditorApplication.timeSinceStartup;
            if(index<(walking?Route.Length:6)){if(!walking)Place();return;}
            Validate();
            File.WriteAllText("Logs/"+(walking?"walk-route-complete":"walk-capture-complete")+".txt",$"{DateTime.UtcNow:O}\nDistance={distance:F2}m\nWaypoints={index}\n");
            Cleanup();EditorApplication.isPlaying=false;
        }
        catch(Exception e){File.WriteAllText("Logs/walk-review-error.txt",e.ToString());Debug.LogException(e);Cleanup();EditorApplication.isPlaying=false;}
    }
    ...
    static void Validate()
    {
        ...
        if(Object.FindAnyObjectByType<LittleSwitch.ShopGame>())throw new Exception("Gameplay systems found in environment scene");
    ...
    public static void BuildWindows()
    {
        ...
        var report=BuildPipeline.BuildPlayer(new BuildPlayerOptions{scenes=new[]{WalkableWorkshopBuilder.ScenePath},locationPathName="../Builds/WalkableWorkshop/Little Switch Workshop.exe",target=BuildTarget.StandaloneWindows64,options=BuildOptions.None});
        File.WriteAllText("Logs/walk-build.txt",report.summary.result+"\nErrors="+report.summary.totalErrors);
